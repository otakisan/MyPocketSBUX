...
class StoresTableViewController: StoresBaseTableViewController, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating {
    ...
    func insertNewStoreToLocal(newStoreData : NSArray) {
        ...
        for newStore in newStoreData {
            ...
            entity.closingTimeSaturday = DateUtility.dateFromSqliteDateString(newStore["closing_time_saturday"] as? String ?? "") ?? NSDate(timeIntervalSince1970: 0)
            entity.openingTimeHoliday = DateUtility.dateFromSqliteDateString(newStore["opening_time_holiday"] as? String ?? "") ?? NSDate(timeIntervalSince1970: 0)
