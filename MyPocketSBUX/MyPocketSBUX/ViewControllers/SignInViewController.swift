...
class SignInViewController: UIViewController, AccountSettingsViewControllerDelegate {
    ...
    @IBAction func touchUpInsideSignInButton(sender: UIButton) {
        ...
        if let user = AccountManager.instance.signIn(self.myPocketIDTextField.text!, password: self.passwordTextField.text!) {
            UIAlertView(title: "\(user.myPocketId) : \("SignedIn".localized())", message: "Succeeded".localized(), delegate: nil, cancelButtonTitle: "Close".localized()).show()
            
            // ログイン後、通知許可を求める
            AccountManager.instance.registerForRemoteNotifications()
        }
        else {
            UIAlertView(title: "FailedToSignIn".localized(), message: "Failed".localized(), delegate: nil, cancelButtonTitle: "Close".localized()).show()
        }
