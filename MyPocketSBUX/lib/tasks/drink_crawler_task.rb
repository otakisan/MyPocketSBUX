...
class Tasks::DrinkCrawlerTask
  def self.execute
    Rails.logger.debug("DrinkCrawlerTask start...")
    #crawlstorebystoreurl("http://www.starbucks.co.jp/store/search/detail.php?id=888")
    crawldrink
    Rails.logger.debug("DrinkCrawlerTask end...")
  end
  ...
  def self.product(node)
    ...
    # ミルク定義
    p milk = ["ラテ", "キャラメル", "カプチーノ", "ココア", "モカ", "ミスト", "フラペチーノ" ].any? { |milkkeyword| product_name.include?(milkkeyword) && !(["マンゴー", "ディスカバリーズ"].any?{|nakeyword| product_name.include?(nakeyword)}) } ? "whole/two-percent/nonfat/soy" : "na"
    ...
    p "category:" + category
    p "jan_code:" + jan_code
    p "price:" + (price_int = detail_prices.first.nil? ? 0 : detail_prices.first.delete("¥").to_i).to_s
    p "special:" + product_special
    p "notes:" + (detail_string = detail_names.join("\t"))
